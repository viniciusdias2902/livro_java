...
mecanismo novo participa, é um construtor comum recebendo uma
interface, e o efeito no teste é imediato:
...
acoplamento cai, e sobra a pergunta prática: se ninguém cria
...
4. Acrescente ao container a detecção de ciclo: `A` que depende de `B` que
   depende de `A` hoje estoura a pilha de chamadas. Transforme o
